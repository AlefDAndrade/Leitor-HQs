	...
	def get_segments(self):
		...
		while self.segments is None or len(self.segments) > 500:
			...
			for dline in dlines[0]:
				# Formatos observados do LSD variam entre versões/plataformas do
				# OpenCV: normalmente cada item vem como [[x0,y0,x1,y1]] (shape
				# (1,4)), mas em algumas versões vem "achatado" direto como
				# [x0,y0,x1,y1] (shape (4,)). np.reshape(-1) normaliza os dois
				# casos pra uma lista simples de 4 números, evitando o erro
				# "invalid index to scalar variable" no segundo caso.
				coords = np.asarray(dline).reshape(-1)
